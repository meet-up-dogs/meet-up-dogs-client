...
const SignUp = (props) => {
    ...
    const signUpHandler = async (e) => {
        ...
        try {
            const axiosResp = await axios.post("http://localhost:4000/signup", inputSignUp)
    ...
    return (
        <>
            <form action="/signup" method="post" id="signUpForm" onSubmit={(e) => { signUpHandler(e) }}>
                <h2> Sign Up </h2>
                <TextField
                ...
                />
                <TextField
                    name="email" label="E-Mail" variant="standard" value={inputSignUp.email} sx={margin} 
                    
                    onChange={(e) => setInputSignUp({...inputSignUp, email: e.target.value})}
                />
                <TextField
                    name="password" label="Password" variant="standard" value={inputSignUp.password}  sx={margin}
